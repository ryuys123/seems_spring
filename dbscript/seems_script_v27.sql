...
CREATE TABLE TB_PERSONALITY_ANALYSIS (
                                         ...
                                         MBTI_TITLE VARCHAR2(100 CHAR), -- ✨ 컬럼명 및 크기 변경
                                         CREATED_AT DATE DEFAULT SYSDATE NOT NULL
);

-- 테이블 및 각 컬럼에 대한 설명 추가
...
COMMENT ON COLUMN TB_PERSONALITY_ANALYSIS.PERSONALITY_TEST_ID IS '어떤 종류의 테스트인지 식별하는 번호 (예: 1=MBTI)';
COMMENT ON COLUMN TB_PERSONALITY_ANALYSIS.USER_ID IS '검사를 수행한 사용자의 ID';
COMMENT ON COLUMN TB_PERSONALITY_ANALYSIS.RESULT IS '분석 결과 (예: INFP)';
COMMENT ON COLUMN TB_PERSONALITY_ANALYSIS.DESCRIPTION IS '결과에 대한 상세 설명';
COMMENT ON COLUMN TB_PERSONALITY_ANALYSIS.MBTI_TITLE IS 'MBTI 유형별 칭호'; -- ✨ 주석 내용 변경
COMMENT ON COLUMN TB_PERSONALITY_ANALYSIS.CREATED_AT IS '결과 생성 시간';
-- 15 수정
