...
public class FaceRecognitionUtil {
    ...
    public boolean testConnection() {
        try {
            ...
            log.info("Flask 서버 연결 테스트 시작: URL={}", url);

            ResponseEntity<Map> response = restTemplate.getForEntity(url, Map.class);
            log.info("Flask 서버 연결 테스트: status={}, body={}", response.getStatusCode(), response.getBody());

            boolean isConnected = response.getStatusCode() == HttpStatus.OK;
            if (isConnected) {
                log.info("Flask 서버 연결 성공");
            } else {
                log.warn("Flask 서버 연결 실패: status={}", response.getStatusCode());
            }

            return isConnected;
        } catch (Exception e) {
            log.error("Flask 서버 연결 실패: {}", e.getMessage(), e);
            return false;
        }
    }
}